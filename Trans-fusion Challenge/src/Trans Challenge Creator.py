...
class Application:
    ...
    def createUI(self):
        ...
        self.lblSuccess = Label(self.root, text="Generation Completed!", font=self.bigFont, bg="green", fg="white")
    ...
    def saveToFile(self):
        ...
        messagebox.showinfo("Finished!", "Sylveons added to first empty slots in boxes.")
        """start = time.time()
        while time.time() < start + 1:
            self.lblSuccess.place(x=self.W/2-self.lblSuccess.winfo_reqwidth()/2, y=self.H/3*2)
            self.btnRun.configure(state="disabled")
            self.root.update()

        self.lblSuccess.place_forget()
        self.checkFields()
            """
